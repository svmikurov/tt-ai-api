...
def process_ml_request(self, data: dict):
    ...
    try:
        response = requests.post(AI_API_URL, json={'query': query}, timeout=60)
        ...
        if response.status_code == 200:
            ...
            logger.warning('AI API busy (429), retrying in 3 seconds...')
        ...
        raise self.retry(countdown=5, max_retries=3) from err

    except requests.exceptions.ConnectionError as err:
        logger.error('Cannot connect to AI API, retrying...')
        raise self.retry(countdown=5, max_retries=5) from err
